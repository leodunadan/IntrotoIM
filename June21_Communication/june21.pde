...
PFont font;
PImage diver;
float motion; //motion would be the vertical position of the racket that will be
//determined by the photoresistor
float place = width/2; //set the horizotan postition of the driver
...
void setup() {
  ...
  //port = new Serial(this, (insert port name), 9600);
  port.bufferUntil('\n');   
...
void serialEvent(Serial port) {
  motion = float(port.readStringUntil('\n')); //attaches "motion" to 
  //the value coming from the photoresistor
}
...
void drawRacket() {  //draws the bar you move
  fill(racketColour);
  rectMode(CENTER);
  rect(place, motion, racketWidth, racketHeight); // the rectangle
  //should stay at the same postion on the x axis and its y axis postion
  //should move depending on the photoresisitor values.
}
...
void racketBounce() {    //controls the bouncing off the diver off the racket
  float overhead = motion - place;
  if ((diverX+(diverSize/2) > place-(racketWidth/2)) && (diverX-(diverSize/2) < place+(racketWidth/2))) {
    if (dist(diverX, diverY, diverX, motion)<=(diverSize/2)+abs(overhead)) {
      makeBounceBottom(motion);
      // racket moving up
      if (overhead<0) {
        diverY+=overhead;
