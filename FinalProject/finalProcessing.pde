//Leo El-azhab
//Final
//Processing code component
//This is a game where you have to keep the diver from sinking for as long as you can
//Instructions: use buttons to move the racket and stop diver from sinking 
...
float gravity = 0.1;                        //initialising all the values
...
void setup() {
  ...
  diverY=height/4;
  String[] fontList = PFont.list();         //setting up the font
  printArray(fontList);
...
void keepInScreen() {
  ...
  if (diverY+(diverSize/2) > height) { 
    ...
    if (diverY < height/2) {                        //checks for the position of the diver and sends signal
      port.write('H');                             // high or low depending on the if statement
    } else {
      port.write('L');
    }
  ...
  void drawRacket() {                                 //draws the bar you move
    fill(racketColour);
    rectMode(CENTER);
    rect(positionX, positionY, racketWidth, racketHeight);
    if (left) {
      positionX-= amountX;                           //changes position of the bar according to the button you press
    }
    if (right) {
      positionX+= amountX;
    }
